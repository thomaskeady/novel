...
/*
void DetectNode::configCallback(aruco_detect::DetectorParamsConfig & config, uint32_t level)
{
    if (level == 0xFFFFFFFF) {
        return;
    }
...
bool DetectNode::enableDetectionsCallback(std_srvs::SetBool::Request &req,
                                std_srvs::SetBool::Response &res)
{
...
DetectNode::DetectNode(ros::NodeHandle & nh) : it(nh)
{
    ...
    // Camera intrinsics
    float data[9] = {700.054181, 0.0, 290.495406, 0.0, 661.978275, 254.169448, 0.0, 0.0, 1.0 };
    cameraMatrix = cv::Mat(3, 3, CV_64F, data);

    // distortion coefficients
    float data2[5] = { 0.138784, -0.598216, 0.000282, 0.002860, 0.0 };
    distortionCoeffs = cv::Mat(1, 5, CV_64F, data2);
    ...
    */
    ...
    service_enable_detections = nh.advertiseService("enable_detections",
                        &DetectNode::enableDetectionsCallback, this);

    /*
    nh.param<double>("adaptiveThreshConstant", detectorParams->adaptiveThreshConstant, 7);
    nh.param<int>("adaptiveThreshWinSizeMax", detectorParams->adaptiveThreshWinSizeMax, 53);
    nh.param<int>("adaptiveThreshWinSizeMin", detectorParams->adaptiveThreshWinSizeMin, 3);
    nh.param<int>("adaptiveThreshWinSizeStep", detectorParams->adaptiveThreshWinSizeStep, 4);
    nh.param<int>("cornerRefinementMaxIterations", detectorParams->cornerRefinementMaxIterations, 30);
    nh.param<double>("cornerRefinementMinAccuracy", detectorParams->cornerRefinementMinAccuracy, 0.01);
    ...
    nh.param<double>("minMarkerPerimeterRate", detectorParams->minMarkerPerimeterRate, 0.1);
    ...
    nh.param<double>("polygonalApproxAccuracyRate", detectorParams->polygonalApproxAccuracyRate, 0.01);
    */
