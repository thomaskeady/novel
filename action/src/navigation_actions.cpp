...
void Action::runAway(int x_run, int y_run) {
  ...
  if (map_known && pose_known && !execute_plan) {
    ...
    while (execute_plan) {
      ...
      // ROS_INFO_STREAM(weights[goal_y][goal_x]);
...
void Action::rotateForBetterView(int x_rot, int y_rot) {
  ...
  if (map_known && pose_known && !execute_plan) {
    ...
    double dist_m = grid_dist * map_resolution; // distance in map coordinate
    ...
    while (execute_plan) {
      ...
      if (dist < dist_m) {
        ...
        goal.target_pose.pose.orientation.z = angle;
        goal.target_pose.pose.orientation.w = 1;
        execute_plan = false; // even if amcl cannot simply rotate robot in place, plan is done
      }
